    ...
    <el-table v-loading="loading" :data="tagList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['blog:tag:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['blog:tag:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除标签管理编号为"' + _ids + '"的数据项？').then(function() {
    return delTag(_ids);
  }).then(() => {
    getList();
